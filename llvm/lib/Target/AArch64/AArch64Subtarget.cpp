...
bool 
AArch64Subtarget::isRegInClass(const MachineInstr *MI,
                                const Register &Reg,
...
bool
AArch64Subtarget::canLowerToZeroCycleRegMove(const MachineInstr *CopyMI,
  ...
  if (isRegInClass(CopyMI, DestReg, &AArch64::FPR32RegClass) &&
    ...
    return hasZeroCycleRegMoveFPR32() || 
...
/// NOTE: must maintain consistency with `AArch64InstrInfo::copyPhysReg`.
bool
AArch64Subtarget::canLowerToZeroCycleRegZeroing(const MachineInstr *CopyMI,
                                      const Register &DestReg,
                                      const Register &SrcReg) const {
  if (isRegInClass(CopyMI, DestReg, &AArch64::GPR32allRegClass) &&
      isRegInClass(CopyMI, SrcReg, &AArch64::GPR32allRegClass) &&
      DestReg != AArch64::WZR) {
    return AArch64::WZR == SrcReg && 
            hasZeroCycleZeroingGP();
  }
